...
class PS4Controller:
    def __init__(self):
        ...
        self.target_joyaxes = [0.] * self.num_axes
        self.last_joyaxes = [0.] * self.num_axes
    ...
    def get_joystick_state(self):
        ...
        for i in range(self.num_axes):
            self.target_joyaxes[i] = self.joystick.get_axis(i)
            if self.target_joyaxes[i] == self.last_joyaxes[i]:
                processed_axes.append(self.last_joyaxes[i])
            else:
                processed_axes.append(
                    self.ramped_vel(
                        self.last_joyaxes[i],
                        self.target_joyaxes[i],
                        self.last_send_time,
                        time.time()
                    )
                )
