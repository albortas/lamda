...
from src.cmass.CMass import CMass
...
class WalkGaitController():       
    def __init__(self, x_offset, default_height, legs, default_stance, default_frame):
        self.legs = legs
        self.CG = np.zeros(3)
        self.cmass = CMass(default_frame, legs)
    ...
    def step(self, state):
        ...
        self.CG = self.cmass.CM_calculation_o(state.angles_foot)
        ...
        state.dCG = self.cmass.CG_distance(state.foot_abs[0], state.foot_abs[1], state.foot_abs[2], state.CGabs[0], state.CGabs[1], state.stance)
        print(self.CG)
